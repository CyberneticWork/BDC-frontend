...
const CashFlowStatement = () => {
  ...
  const calculateTotals = (data) => {
    const operatingCashFlow = Object.values(data.operatingActivities).reduce(
      (sum, val) => sum + val,
      0
    );
    const investingCashFlow = Object.values(data.investingActivities).reduce(
      (sum, val) => sum + val,
      0
    );
    const financingCashFlow = Object.values(data.financingActivities).reduce(
      (sum, val) => sum + val,
      0
    );
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <Icon
            className={`h-4 w-4 sm:h-5 sm:w-5 text-${color}-600 flex-shrink-0`}
          />
          {isExpanded ? (
          ...
          <span
            className={`font-semibold text-${color}-900 text-sm sm:text-base truncate`}
          >
            {title}
          </span>
        </div>
  ...
  const LineItem = ({ label, amount, previousAmount, isNegative = false }) => {
    ...
    return (
      <div className="flex justify-between py-3 px-4 sm:px-12 hover:bg-gray-50 border-l border-gray-200">
        <span className="text-gray-700 text-sm sm:text-base pr-2 truncate">
          {label}
        </span>
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-6">
        ...
        <MetricCard
          ...
          }${formatCurrency(currentTotals.netChangeInCash)}`}
          icon={
            currentTotals.netChangeInCash >= 0 ? ArrowUpRight : ArrowDownRight
          }
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
        ...
        <div className="bg-blue-50 px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
          <div className="flex justify-between items-center">
            ...
            <span className="font-bold text-blue-900 text-sm sm:text-base">
              {formatCurrency(currentTotals.beginningCash)}
            </span>
          </div>
        </div>
        ...
        <div className="border-b border-gray-100">
          ...
          {expandedSections.operating && (
            <div className="bg-green-25">
              {Object.entries(cashFlowData.operatingActivities).map(
                ([key, value]) => (
                  <LineItem
                    key={key}
                    label={key
        ...
        <div className="border-b border-gray-100">
          ...
          {expandedSections.investing && (
            <div className="bg-blue-25">
              {Object.entries(cashFlowData.investingActivities).map(
                ([key, value]) => (
                  <LineItem
                    key={key}
                    label={key
        ...
        <div className="border-b border-gray-100">
          ...
          {expandedSections.financing && (
            <div className="bg-purple-25">
              {Object.entries(cashFlowData.financingActivities).map(
                ([key, value]) => (
                  <LineItem
                    key={key}
                    label={key
        ...
        <div
          className={`${
            currentTotals.netChangeInCash >= 0 ? "bg-green-100" : "bg-red-100"
          } px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200`}
        >
          <div className="flex justify-between items-center">
            <span
        ...
        <div className="bg-blue-100 px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex justify-between items-center">
            ...
            <span className="font-bold text-blue-900 text-base sm:text-lg">
              {formatCurrency(currentTotals.endingCash)}
            </span>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`font-semibold text-sm sm:text-base ${
                    currentTotals.operatingCashFlow / 100000 > 0.15
                      ? "text-green-600"
                      : "text-yellow-600"
                  }`}
                >
                  {((currentTotals.operatingCashFlow / 100000) * 100).toFixed(
                    1
                  )}
                  %
                </span>
              ...
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`font-semibold text-sm sm:text-base ${
                    currentTotals.operatingCashFlow - 15000 >= 0
                      ? "text-green-600"
                      : "text-red-600"
                  }`}
                >
