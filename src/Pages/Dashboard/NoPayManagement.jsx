...
const NoPayManagement = () => {
  ...
  // ✅ ONE DATE FOR BOTH (Filter + Generate)
  ...
  const fetchNoPayRecords = async () => {
    ...
    try {
      ...
      /**
       * ✅ DATE OVERRIDE
       * If selectedDate exists => only date filter
       * Else => month/year filter
       */
      if (selectedDate) {
        ...
        if (month) {
          ...
          if (m >= 1 && m <= 12) params.month = m;
        }
        if (year) params.year = year;
      }

      console.log("NOPAY PARAMS SENT =>", params);

      const response = await NoPayService.getAllRecords(params);

      // Laravel paginate => { data: [...], total: ... }
  ...
  // ✅ Generate uses the SAME selectedDate
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    title="Clear date (go back to month view)"
                  >
                    Clear
                  </button>
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  Date select කරොත් table එක ඒ date එකට විතරක්. Clear කරොත් Month/Year view.
                </p>
              </div>
