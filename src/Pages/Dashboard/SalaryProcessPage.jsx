...
const SalaryProcessPage = () => {
  ...
  // ✅ Buttons = UI highlight only (NO filtering)
  ...
  // Load allowances/deductions on mount
  useEffect(() => {
  ...
  const fetchSalaryData = async () => {
    ...
    try {
      ...
      console.log("DEBUG FIRST EMP:", rows?.[0]);
      setEmployeeData(rows);

      // ✅ no client filtering -> show all rows returned by backend
  ...
  // ✅ Apply Filters = only fetch (no EPF/search filtering)
      ...
      const worksheetData = (employees || []).map((employee) => {
        ...
        if (bonuses && bonuses.length > 0) {
          bonuses.forEach((bonus) => {
            ...
            row[`Bonus Name`] = bonus.bonus_name; // or bonus.name
            row[`Amount (LKR)`] = 0;
          });
        }
                ...
                <div className="relative">
                  ...
                  {isLoadingDepartments ? (
                    ...
                    <select
                      ...
                      className={`appearance-none w-full pl-10 pr-10 py-2 border border-gray-300 rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white shadow-sm ${!selectedCompany ? "bg-gray-100 cursor-not-allowed" : ""
                        }`}
                    >
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="flex items-center">
                ...
                <p
                  className={`font-semibold text-lg ${status === "Processed"
                      ? "text-green-700"
                      : "text-yellow-700"
                    }`}
                >
                ...
                {(employee.bonuses || []).length > 0 ? (
                  <div className="space-y-2">
                    {employee.bonuses.map((b, idx) => (
                      <div key={idx} className="flex justify-between text-sm">
                        <span className="text-gray-600">
                          {b.name} <span className="text-xs text-gray-400">({b.code})</span>
                        </span>
                        <span className="font-semibold">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold text-red-600">
                      {Number(employee.salary_breakdown?.epf_employee_deduction || 0).toLocaleString()}
                    </span>
                  </div>
  ...
  <div className="text-sm font-bold">{holidayOt.toLocaleString()}</div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-red-600">
                      {Number(employee.salary_breakdown?.stamp || 0).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {displayedData.map((employee) => {
                  ...
                  console.log("BONUSES RAW:", employee.emp_no, employee.bonuses);
                  ...
                  const totalBonuses = Number(employee.salary_breakdown?.total_bonus || 0);
                        ...
                        <div className="flex flex-col space-y-1">
                          {employee.bonuses?.map((bonus) => (
                            <div key={bonus.id} className="flex justify-between">
                              ...
                              <span className="font-medium">
                                {parseFloat(bonus.amount || 0).toLocaleString()}
                              </span>
                            </div>
                          ))}

                          <div className="font-semibold border-t mt-1 pt-1">
                            Total: {totalBonuses.toLocaleString()}
                          </div>
                        </div>
                      </td>
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <div className="rounded-lg border border-gray-200 bg-white p-3">
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex justify-between items-center">
                                <span className="text-xs text-gray-600">BRA Allowance</span>
                                <span className="text-xs font-semibold tabular-nums">
                                  {Number(employee.salary_breakdown?.br_allowance || 0).toLocaleString()}
                                </span>
                              </div>
