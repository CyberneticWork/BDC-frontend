...
const EmployeeDocumentsModal = ({ isOpen, onClose, review }) => {
  ...
  const getEmployeeDocuments = () => {
    if (!linkedTaskId) {
      console.log('No linked task ID found in review:', review);
      return [];
    }
    
    const task = PMSDummyDataStore.getTaskById(linkedTaskId);
    if (!task) {
      console.log('Task not found for ID:', linkedTaskId);
      return [];
    }
    ...
    task.assigneeUpdates?.forEach(assignee => {
      console.log('Processing assignee:', assignee.employeeId, 'with updates:', assignee.updates);
      assignee.updates.forEach(update => {
        if (update.documentName) {
          console.log('Found document:', update.documentName);
          allDocuments.push({
            ...update,
    ...
    console.log('All documents found:', allDocuments);
  ...
  const filteredDocuments = documents.filter(doc => {
    ...
    // Filter by document type if needed in future
...
In a real application, this would be the actual file content uploaded by the employee.`;
  ...
  // Improved download function that creates and downloads a file
  const handleDownload = (document) => {
    ...
    setTimeout(() => {
      try {
        ...
        // Set appropriate MIME type based on document type
        ...
        <div className="p-6">
          {linkedTaskId ? (
              ...
              {filteredDocuments.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 mt-2">
                    {searchTerm ? 'No documents match your search criteria.' : 'No documents have been submitted for this review yet.'}
                  </p>
                  {linkedTaskId && (
                    <p className="text-xs text-gray-400 mt-2">
                      Task ID: {linkedTaskId} - Check console for debugging info.
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 mt-2">
                This review is not linked to a specific KPI task. Documents can only be viewed for reviews linked to tasks.
              </p>
            </div>
          )}
        </div>
