...
// import * as EmployeeWiseAllowanceService from "@services/EmployeeWiseAllowanceService";
import * as EmployeeWiseAllowanceService from "../../services/EmployeeWiseAllowanceService";
import moment from "moment";

function EmployeeWiseAllowance() {
    ...
    const filteredData = useMemo(() => {
        ...
        return data.filter(rec =>
            rec.allowance_code.includes(searchTerm) ||
            rec.allowance_name.includes(searchTerm) ||
    ...
    const fetchData = useCallback(async () => {
        setIsLoading(true);
        try {
            const res = await EmployeeWiseAllowanceService.getAll();
            setData(res);
        } catch (err) {
            console.error(err);
    ...
    const createAllowanceRecord = useCallback(async (data, onFinished) => {
        try {
            const res = await EmployeeWiseAllowanceService.createOne(data);
            setData(prev => [...prev, res]);
            toast.success("Allowance record created successfully.");
        } catch (err) {
            console.error(err);
            toast.error(err instanceof Error ? err.message : "An unknown error occurred!");
        } finally {
    ...
    const updateAllowanceRecord = useCallback(async (id, data, onFinished) => {
        try {
            const res = await EmployeeWiseAllowanceService.updateOne(id, data);
            console.log(res);
            setData(prev =>
    ...
    const deleteAllowanceRecord = useCallback(async (onFinished) => {
        try {
            const res = await EmployeeWiseAllowanceService.deleteOne(selectedForDelete.id);
    ...
    const handleAddNewAllowanceClick = useCallback(() => {
                        ...
                        {employee && (<>
                            <div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            updateForm("allowance_code", e.target.value)
                                        }
                                        className={`w-full px-4 py-3 border "border-gray-200" rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all`}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        updateForm("allowance_name", e.target.value)
                                    }
                                    className={`w-full px-4 py-3 border "border-gray-200" rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all`}
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={form.allowance_description}
                                    onChange={(e) => updateForm("allowance_description", e.target.value)}
                                    className={`w-full px-4 py-3 border "border-gray-200" rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all`}
     ...
     * @property {import("../../services/EmployeeWiseAllowanceService").EmployeeWiseAllowance} selectedRecord
    ...
    function UpdateFormModal({ selectedRecord, setSelectedRecord, onSubmit, onClose }) {
        ...
        const handleSubmit = useCallback(() => {
            ...
            const payload = {
                ...
                allowance_code: selectedRecord.allowance_code,
                allowance_name: selectedRecord.allowance_name,
                allowance_description: selectedRecord.allowance_description,
                        ...
                        <>
                            <div>
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        value={selectedRecord.allowance_code}
                                        onChange={(e) =>
                                            updateForm("allowance_code", e.target.value)
                                        }
                                        className={`w-full px-4 py-3 border "border-gray-200" rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all`}
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={selectedRecord.allowance_name}
                                    onChange={(e) =>
                                        updateForm("allowance_name", e.target.value)
                                    }
                                    className={`w-full px-4 py-3 border "border-gray-200" rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all`}
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={selectedRecord.allowance_description}
                                    onChange={(e) => updateForm("allowance_description", e.target.value)}
                                    className={`w-full px-4 py-3 border "border-gray-200" rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all`}
                                    placeholder="Enter allowance description"
                                />
        ...
        <div className="w-full flex flex-col flex-1 overflow-hidden">
            ...
            <div className="mb-8 shrink-0">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-4">
                    <div className="flex flex-col sm:flex-row gap-3">
                        <button
                            onClick={handleAddNewAllowanceClick}
            ...
            {createFormVisible &&
                <CreateFormModal
                    ...
                    onSubmit={(data, onFinished) => createAllowanceRecord(data, onFinished)}
                />
            }
            ...
            {selectedRecord &&
                <UpdateFormModal
                    ...
                    onSubmit={(id, data, onFinished) => updateAllowanceRecord(id, data, onFinished)}
                />
            }
            ...
            {selectedForDelete &&
                <DeleteFormModal
                    ...
                    onSubmit={deleteAllowanceRecord}
                />}
        </div>
    );
}

export default memo(EmployeeWiseAllowance);