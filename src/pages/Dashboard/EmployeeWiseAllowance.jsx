...
function EmployeeWiseAllowance() {
  ...
  const loadAssignments = useCallback(async () => {
    ...
    try {
      ...
      const rows = await listAssignedAllowances(params);
      setAssignments(rows);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Remove this allowance assignment?")) return;
    setDeletingId(id);
    try {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex flex-col lg:flex-row lg:items-end gap-3 justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Assign Allowances</h2>
          <p className="text-sm text-gray-600 mt-1">
            Select a predefined allowance, apply to all employees or one employee.
            ...
            <tbody>
              {filtered.map((row) => (
                <tr key={row.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 capitalize">{row.allowance_type || "—"}</td>
                  <td className="px-4 py-3">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-10 text-center text-gray-500">
                    No assignments found. Click Assign Allowance to add.
                  </td>
                </tr>
              )}
            </tbody>
...
function AssignAllowanceModal({ companies, predefined, onClose, onSuccess }) {
  const now = new Date();
  ...
  const selectedAllowance = useMemo(
    () => filteredAllowances.find((a) => String(a.id) === String(allowanceId)),
    [filteredAllowances, allowanceId]
  );

  useEffect(() => {
    if (!selectedAllowance) return;
    const type = String(selectedAllowance.allowance_type || "fixed").toLowerCase();
    setValueType(type === "variable" ? "variable" : "fixed");
    if (type !== "variable") {
      setAmount(String(selectedAllowance.amount ?? ""));
    } else {
      setAmount("");
    }
  }, [selectedAllowance]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!allowanceId) {
      toast.error("Select a predefined allowance");
      return;
    }
    ...
    if (valueType === "variable" && (amount === "" || Number(amount) < 0)) {
      toast.error("Enter amount for variable allowance");
      return;
    }
    ...
    const payload = {
      ...
      company_id: companyId || selectedAllowance?.company_id || null,
    ...
    setIsSaving(true);
    try {
      const res = await assignAllowance(payload);
      toast.success(
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h3 className="text-lg font-semibold">Assign Predefined Allowance</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            >
              ...
              {filteredAllowances.map((a) => (
                <option key={a.id} value={a.id}>
                  {a.allowance_code} — {a.allowance_name} ({a.allowance_type || "fixed"})
          ...
          <div>
            ...
            {selectedAllowance && valueType === "fixed" && (
              <p className="text-xs text-gray-500 mt-1">
                Master amount: {Number(selectedAllowance.amount || 0).toFixed(2)}
              </p>
            )}
          </div>
