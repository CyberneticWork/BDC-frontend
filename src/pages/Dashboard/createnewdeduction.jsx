...
const CreateNewDeduction = () => {
  ...
  const validateForm = () => {
    ...
    if (!showEditModal && !formData.deduction_code)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const deductionData = {
        ...
        amount: parseFloat(formData.amount), // Amount එක යවනවා
  ...
  const handleEdit = async (deduction) => {
    try {
      ...
      setFormData({
        ...
        amount: deduction.amount || "", // Amount එක Set කරනවා
  ...
  const handleToggleDeductionStatus = async (id) => {
    ...
    try {
      ...
      setDeductions((prevDeductions) =>
        prevDeductions.map((d) =>
          d.id === id ? { ...d, status: deduction.status } : d
        )
      );
